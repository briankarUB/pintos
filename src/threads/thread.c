...
thread_exit (void)
{
  ...
  list_remove (&thread_current()->allelemord);
...
/* Invoke function 'func' on all threads in priority order, passing along 'aux'.
   This function must be called with interrupts off. */
void
thread_foreach_ordered (thread_action_func *func, void *aux)
{
  struct list_elem *e;

  ASSERT (intr_get_level () == INTR_OFF);

  for (e = list_begin (&all_list_ordered); e != list_end (&all_list_ordered);
       e = list_next (e))
    {
      struct thread *t = list_entry (e, struct thread, allelemord);
      func (t, aux);
    }
}
